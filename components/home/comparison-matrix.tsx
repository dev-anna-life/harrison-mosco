...
export function ComparisonMatrix() {
  ...
  return (
    <section className="py-16 sm:py-32 bg-[#070b13] border-b border-slate-800 text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <Reveal type="scale" delay={200} duration={0.8}>
          <div className="max-w-5xl mx-auto bg-[#0f172a] border border-slate-800 rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl">
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-800">
              {/* Left: Traditional */}
              <div className="p-4 sm:p-10 bg-red-950/10">
                <div className="flex items-center gap-2 text-red-400 font-black text-xs sm:text-sm uppercase tracking-wider mb-4 sm:mb-6">
                  <X className="w-4 h-4 sm:w-5 sm:h-5 text-red-500 shrink-0" />
                  <span>Traditional Roadside Agents</span>
                </div>
                <ul className="space-y-4 sm:space-y-6">
                  {comparisons.map((c, i) => (
                    <li key={i} className="space-y-0.5 sm:space-y-1">
                      <strong className="text-slate-200 block font-black text-xs sm:text-base">{c.feature}</strong>
                      <p className="text-slate-400 leading-relaxed text-xs sm:text-sm font-normal">{c.traditional}</p>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="p-4 sm:p-10 bg-emerald-950/10 border-t md:border-t-0 border-emerald-500/20">
                <div className="flex items-center gap-2 text-[#FDC902] font-black text-xs sm:text-sm uppercase tracking-wider mb-4 sm:mb-6">
                  <Check className="w-4 h-4 sm:w-5 sm:h-5 text-[#FDC902] shrink-0" />
                  <span>Harrison Mosco Platform</span>
                </div>
                <ul className="space-y-4 sm:space-y-6">
                  {comparisons.map((c, i) => (
                    <li key={i} className="space-y-0.5 sm:space-y-1">
                      <strong className="text-white block font-black text-xs sm:text-base">{c.feature}</strong>
                      <p className="text-slate-300 leading-relaxed text-xs sm:text-sm font-medium">{c.harrison}</p>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {/* Bottom Bar */}
            <div className="p-4 sm:p-6 bg-[#0a0e17] border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs sm:text-sm">
              <span className="text-slate-300 font-bold text-center sm:text-left">
                Ready to launch your Nigerian business with modern infrastructure?
              </span>
              <a
                href="/limited"
                className="w-auto inline-flex items-center justify-center px-5 py-2.5 sm:px-7 sm:py-3 bg-[#FDC902] hover:bg-amber-400 text-slate-950 font-black rounded-xl transition-all shadow-[0_4px_20px_rgba(253,201,2,0.25)] text-xs sm:text-sm text-center shrink-0"
              >
                Explore Limited Packages &rarr;
              </a>
            </div>
          </div>
        </Reveal>
