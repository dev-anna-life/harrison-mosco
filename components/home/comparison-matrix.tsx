        ...
        <Reveal type="scale" delay={200} duration={0.8}>
          <div className="max-w-5xl mx-auto bg-[#0f172a] border border-slate-800 rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl">
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-800">
              {/* Left: Traditional */}
              <div className="p-5 sm:p-12 bg-red-950/10">
                <div className="flex items-center gap-2 text-red-400 font-black text-xs sm:text-sm uppercase tracking-wider mb-5 sm:mb-8">
                ...
                <ul className="space-y-5 sm:space-y-8">
                  {comparisons.map((c, i) => (
                    <li key={i} className="space-y-1">
                      <strong className="text-slate-200 block font-black text-sm sm:text-lg">{c.feature}</strong>
                      <p className="text-slate-400 leading-relaxed text-xs sm:text-base font-normal">{c.traditional}</p>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="p-5 sm:p-12 bg-emerald-950/10 border-t md:border-t-0 border-emerald-500/20">
                <div className="flex items-center gap-2 text-[#FDC902] font-black text-xs sm:text-sm uppercase tracking-wider mb-5 sm:mb-8">
                ...
                <ul className="space-y-5 sm:space-y-8">
                  {comparisons.map((c, i) => (
                    <li key={i} className="space-y-1">
                      <strong className="text-white block font-black text-sm sm:text-lg">{c.feature}</strong>
                      <p className="text-slate-300 leading-relaxed text-xs sm:text-base font-medium">{c.harrison}</p>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {/* Bottom Bar */}
            <div className="p-5 sm:p-8 bg-[#0a0e17] border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs sm:text-base">
              ...
              <a
                href="/limited"
                className="w-full sm:w-auto px-6 sm:px-8 py-3.5 bg-[#FDC902] hover:bg-amber-400 text-slate-950 font-black rounded-xl transition-all shadow-[0_4px_20px_rgba(253,201,2,0.25)] text-xs sm:text-sm text-center"
              >
