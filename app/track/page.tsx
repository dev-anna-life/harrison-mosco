    ...
    <div className="bg-[#0a0e17] text-white min-h-screen relative overflow-hidden">
      ...
      <section className="py-20 sm:py-28 border-b border-slate-800 relative z-10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-6">
          ...
          <Reveal type="scale" delay={300} duration={0.8}>
            <form onSubmit={handleSearch} className="max-w-xl mx-auto mt-8 flex flex-col sm:flex-row gap-3">
              <input
                type="text"
                required
                value={query}
                ...
                className="flex-1 px-6 py-4 bg-[#0e1626] border-2 border-slate-700 focus:border-[#FDC902] rounded-2xl text-white font-bold text-base focus:outline-none transition-colors"
              />
              <button
                type="submit"
                disabled={loading}
                className="px-8 py-4 bg-[#FDC902] hover:bg-amber-400 text-slate-950 font-black rounded-2xl text-base transition-all shadow-[0_8px_25px_rgba(253,201,2,0.25)] flex items-center justify-center gap-2 shrink-0"
              >
      ...
      <section className="py-20 bg-[#070b13]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          {error && (
            <div className="p-6 rounded-3xl bg-rose-950/40 border border-rose-500/50 text-rose-300 text-center font-bold flex items-center justify-center gap-3">
          ...
          {result && (
            <div className="bg-[#0f172a] rounded-3xl border border-slate-800 p-8 sm:p-12 shadow-2xl space-y-8">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b border-slate-800 gap-4">
                ...
                <div className="text-left sm:text-right">
                  <span className="inline-block px-4 py-1.5 rounded-full bg-emerald-950/50 border border-emerald-500/40 text-emerald-400 text-xs font-black uppercase tracking-wider">
                    {result.status}
                  </span>
                    ...
                    >
                      <div className="mt-0.5">
                        {m.completed ? (
                          ...
                          <Clock className="w-5 h-5 text-[#FDC902] shrink-0 animate-spin" />
                        ) : (
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <strong className="text-sm font-bold block">{m.title}</strong>
                          {m.date && <span className="text-xs font-mono font-medium">{m.date}</span>}
                        </div>
