...
export default function TrackApplicationPage() {
  ...
  return (
    <div className="bg-[#0a0e17] text-white min-h-screen relative overflow-hidden">
      ...
      <section className="py-10 sm:py-24 border-b border-slate-800 relative z-10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4 sm:space-y-6">
          ...
          <Reveal type="scale" delay={300} duration={0.8}>
            <form onSubmit={handleSearch} className="max-w-xl mx-auto mt-5 sm:mt-8 flex flex-col sm:flex-row gap-2.5 sm:gap-3 items-center">
              <input
                type="text"
                required
                value={query}
                ...
                className="w-full sm:flex-1 px-3.5 py-2.5 sm:px-5 sm:py-3.5 bg-[#0e1626] border-2 border-slate-700 focus:border-[#FDC902] rounded-xl text-white font-bold text-xs sm:text-sm focus:outline-none transition-colors"
              />
              <button
                type="submit"
                disabled={loading}
                className="w-auto inline-flex items-center justify-center px-5 py-2.5 sm:px-7 sm:py-3.5 bg-[#FDC902] hover:bg-amber-400 text-slate-950 font-black rounded-xl text-xs sm:text-sm transition-all shadow-[0_6px_20px_rgba(253,201,2,0.2)] gap-2 shrink-0"
              >
      ...
      {/* Result Section */}
      <section className="py-10 sm:py-20 bg-[#070b13]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          {error && (
            <div className="p-3.5 sm:p-5 rounded-2xl bg-rose-950/40 border border-rose-500/50 text-rose-300 text-center font-bold flex items-center justify-center gap-2 text-xs sm:text-sm">
          ...
          {result && (
            <div className="bg-[#0f172a] rounded-2xl sm:rounded-3xl border border-slate-800 p-4 sm:p-7 lg:p-10 shadow-2xl space-y-5 sm:space-y-7">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 sm:pb-5 border-b border-slate-800 gap-2.5 sm:gap-4">
                <div>
                  <span className="text-[10px] sm:text-xs font-black text-[#FDC902] uppercase tracking-widest block mb-0.5">
                    {result.packageChoice} Limited Company
                  </span>
                  <h3 className="text-lg sm:text-2xl font-black text-white">
                    {result.proposedName1} Ltd
                  </h3>
                  <span className="text-[11px] sm:text-xs text-slate-400 font-mono mt-0.5 block font-bold">
                    Ref: {result.reference}
                  </span>
                </div>
                <div className="text-left sm:text-right">
                  <span className="inline-block px-3 py-1 rounded-full bg-emerald-950/50 border border-emerald-500/40 text-emerald-400 text-[10px] sm:text-xs font-black uppercase tracking-wider">
                    {result.status}
                  </span>
                  <span className="block text-[10px] sm:text-xs text-slate-400 mt-1 font-medium">
              ...
              <div className="space-y-3 sm:space-y-5">
                ...
                <div className="space-y-2.5 sm:space-y-3">
                  {result.milestones?.map((m: any, idx: number) => (
                    <div
                      key={idx}
                      className={`p-3 sm:p-4 rounded-xl border flex items-start gap-2.5 sm:gap-3.5 transition-all ${
                        m.completed
                    ...
                    >
                      <div className="mt-0.5">
                        {m.completed ? (
                          ...
                          <Clock className="w-4 h-4 text-[#FDC902] shrink-0 animate-spin" />
                        ) : (
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[11px] sm:text-xs mt-0.5 text-slate-400 leading-relaxed font-normal">
                          {m.description}
                        </p>
                      </div>
                    </div>
              ...
              <div className="pt-4 sm:pt-5 border-t border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5">
